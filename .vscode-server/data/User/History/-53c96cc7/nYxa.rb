...
p database.execute("SELECT count(t.id), avg(t.milliseconds) FROM tracks t JOIN genres g WHERE t.genre_id = g.id AND g.name = 'Rock'").flatten
p database.execute(%(
...
def stats_on(db, genre_name)
  ...
  hash = db.execute("SELECT genres.name, count(t.id), avg(t.milliseconds)
