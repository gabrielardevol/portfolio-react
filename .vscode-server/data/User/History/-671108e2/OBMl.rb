...
wolf_run = timelapse / (mid_distance.to_i + final_distance.to_i)
...
until time2.to_f - time1.to_f >= timelapse do
  ...
  #  wolf_run += wolf_run
  #end
  start_distance = (now / wolf_run).to_i
  mid_distance -= 1
  final_distance.to_i = final_distance_init.to_i - (now / wolf_run).to_i
end
