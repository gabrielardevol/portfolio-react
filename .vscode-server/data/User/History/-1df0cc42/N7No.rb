...
class Game
  def initialize(hash)
    @start_distance = hash[:start_distance]
    @wolf_to_pig_distance = hash[:wolf_to_pig_distance]
    @pig_to_house_distance = hash[:pig_to_house_distance]
    @total_distance = @start_distance + @wolf_to_pig_distance + @pig_to_house_distance
    @game_duration = hash[:game_duration]
    @time1 = Time.now
  end
